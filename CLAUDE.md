...
The release PR is authored by the **httpjpg-bot** GitHub App: `release.yml` mints an installation token with `actions/create-github-app-token` from `HTTPJPG_BOT_APP_ID` + `HTTPJPG_BOT_PRIVATE_KEY` (App permissions: Contents and Pull requests, both read & write). The token falls back to `RELEASE_PLEASE_TOKEN` and then `GITHUB_TOKEN`, but only the first two can start workflow runs — see the required-check note above.
