...
import type { ExtractedColor } from "@httpjpg/spotify";
...
const meta = {
  title: "Widgets/NowPlaying/Spotify",
  ...
  render: () => {
    ...
    useEffect(() => {
      ...
      const fetchNowPlaying = async () => {
        try {
          const response = await fetch("https://localhost:3000/api/spotify/now-playing");
          if (!response.ok) {
