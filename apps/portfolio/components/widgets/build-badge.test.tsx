...
describe("BuildBadge", () => {
  ...
  it("shows the version, the short sha and the deploy age", () => {
    render(
      <BuildBadge version="v2.4.0" commitSha="a1b2c3d4e5f6a7b8" buildTime="2026-08-08T09:00:00Z" />,
    );
  ...
  it("falls back to the release tag when only the version is known", () => {
    render(<BuildBadge version="v2.4.0" />);
  ...
  it("renders an unlinked line when only the build time is known", () => {
    render(<BuildBadge buildTime="2026-08-08T11:00:00Z" />);
  ...
  it("omits the deploy age when the build time is unparseable", () => {
    render(<BuildBadge version="v2.4.0" buildTime="not-a-date" />);
