...
describe("getAllSlugs", () => {
  it("maps links to slug/id/isFolder", async () => {
    mockGet.mockResolvedValueOnce({
      data: {
        links: {
          1: { slug: "home", id: 1, is_folder: false },
          2: { slug: "work", id: 2, is_folder: true },
        },
      },
    });
    const result = await getStoryblokApi().getAllSlugs();
    expect(result).toEqual([
      { slug: "home", id: 1, isFolder: false },
      { slug: "work", id: 2, isFolder: true },
    ]);
  });

  it("defaults a missing slug to an empty string and is_folder to false", async () => {
    mockGet.mockResolvedValueOnce({ data: { links: { 1: { id: 1 } } } });
    const result = await getStoryblokApi().getAllSlugs();
    expect(result).toEqual([{ slug: "", id: 1, isFolder: false }]);
  });

  it("returns an empty array when there are no links", async () => {
    mockGet.mockResolvedValueOnce({ data: {} });
    expect(await getStoryblokApi().getAllSlugs()).toEqual([]);
  });
