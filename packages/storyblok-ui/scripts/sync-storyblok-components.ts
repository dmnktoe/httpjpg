...
async function storyblokRequest<T>(
  ...
  const url = `${STORYBLOK_API}/spaces/${SPACE_ID}${endpoint}`;
 ...
 */
async function getComponent(name: string): Promise<unknown | null> {
  try {
    const response = await storyblokRequest<{ components: unknown[] }>(
      "/components",
    );
...
async function createOrUpdateComponent(
  ...
  const existing = (await getComponent(component.name)) as {
    id: number;
  } | null;
...
function getSbGridItemComponent(): StoryblokComponent {
  return {
    name: "grid_item",
