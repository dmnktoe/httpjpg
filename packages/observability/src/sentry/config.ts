...
export function getSentryConfig(scope: SentryScope): SentryRuntimeConfig {
  ...
  // Prefer the build-time app version (git tag → GitHub release → commit SHA, resolved in
  // next.config.ts). npm_package_version is the unmaintained "0.0.0" placeholder at runtime.
  const release =
