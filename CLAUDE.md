...
`main` is the baseline: a push re-takes every screenshot and publishes the set to the Actions cache, a pull request restores that set and compares. There is no accept step — a pull request that changes rendering fails, uploads the expected/actual/diff images as the `visual-report` artifact, and is merged once a human agrees with the diff.
