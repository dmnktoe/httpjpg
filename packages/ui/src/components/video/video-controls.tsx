...
export function VideoControls({ videoRef, show = true }: VideoControlsProps) {
  ...
  useEffect(() => {
    ...
    // Seed from the live element so a paused (no-autoplay) video shows the
    // bar immediately instead of waiting for a play/pause event.
  ...
  // Paused / not-yet-started: keep the bar visible so play is discoverable
  // (autoplay off, blocked autoplay, touch). Playing: hide until hover.
  const isVisible = !isPlaying || isHovered;
  ...
  return (
    ...
    >
      <Box
        ...
        // Dynamic visibility stays inline so paused chrome is readable without
        // hover, and so a faded-out pause button cannot steal clicks.
        style={{
