...
- **Accepting is a label, not a merge.** `visual-approved` on the pull request makes the job pass; the diff is still computed, reported and uploaded. `.github/workflows/visual-approval-reset.yml` drops the label on every push, so one approval cannot cover a later rendering. The job reads the label from the API rather than the event payload on purpose — "Re-run failed jobs" replays the original payload, where a label added after the fact is invisible.
