...
- **Draft mode searches drafts.** `getSearchIndex({ draftMode })` swaps to `version: "draft"` and **bypasses `unstable_cache` entirely** — the cache is shared across visitors, so a draft written into it would outlive the preview session that fetched it. `/api/search` answers `private, no-store` in draft mode for the same reason. Unpublished stories carry `isDraft`, the palette labels them, and the prompt tells the model to say when it leant on one.
...
- **Answers are lowercase English.** The prompt asks for it and `toAnswerCase` in `lib/search/prompt.ts` enforces it on the way out, because the model capitalises names and sentence openings whatever the prompt says. Applying it per chunk is safe — case folding needs no surrounding context — and it leaves citations and punctuation untouched.
...
- **Story-level Storyblok tags are a separate axis.** `tag_list` says where a story belongs — `Projects` and `Websites` drive the work-list split and the header nav, and they live in `lib/queries/work.ts`. `content.tags` says what it is about. Nothing reads `tag_list` for description: cards and the search index render the curated vocabulary only, so the two never mix.
